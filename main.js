$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < filmResults.length; i++) {
        //ciclo tutta la lunghezza dei risultati
        var imgFilm =
          '<img src="' +
          'https://image.tmdb.org/t/p/w342' +
          filmResults[i].poster_path +
          '">'; //var che conterrà il tag dell'immagine
        if (
          filmResults[i].poster_path != null &&
          filmResults[i].vote_average > 6
        ) {
          //controllo prima che ci sia un'immagine come locandina e che il voto del film sia superiore a 6 (in modo da far uscire i film più votati)
  ...
  var clock = setInterval(function () {
    ...
    if (divNext.length == 0) {
      //controllo che ci sia un div e nel caso non c'è, riprende il div iniziale
  ...
  $('.searchMovie').keypress(function (event) {
    //quando si è in posizione dell'input e viene premuto INVIO
    var testoRicerca = $('.searchMovie').val(); //recupero ciò che viene scritto nell'input
    if (event.which == 13 && testoRicerca != 0) {
      // se viene premuto INVIO (che corrisponde al numero 13 della mappatura dei tasti) e se nell'input c'è scritto qualcosa
      trovaFilm(); //applico la mia funzione creata
      var typeSelect = $('.choise').val(''); //reimposto la select con il valore predefinito
    }
  });
  ...
  $(document).on('mouseenter', '.searchReaults', function () {
    //quando sono con il mouse sul div che contiene la card del film/serie
  ...
  $(document).on('mouseleave', '.searchReaults', function () {
    //quando esco con il mouse dal div che contiene la card del film/serie
  ...
  $('.choise').change(function () {
    //verifico quando viene cambiata l'opzione
    var typeSelect = $('.choise').val(); //creo una var che mi prende il value nell'option dentro il select
    if (typeSelect == '') {
      // se l'opzione scelta è uguale ad una stringa vuota, ovvero è impostato su 'Ordina per: Tutti i risultati' (che non ha val)
      $('.searchReaults').fadeIn(); //mostra tutto
    } else {
      //altrimenti se viene scelto 'Film' oppure 'SerieTV'
      $('.searchReaults').each(function () {
        ...
        if (typeChoise.toLowerCase() == typeSelect.toLowerCase()) {
          //se l'attributo del typeChoise è uguale a una delle opzioni sella select
...
function trovaFilm() {
  ...
  if (typeSelect == '') {
    //se l'opzione scelta è uguale ad una stringa vuota, ovvero è impostato su 'Lingua di ricerca: (default ITA)' (che non ha val)
    var language = 'it'; //la lingua è di default in italiano
  }
  if (typeSelect == 'eng') {
    //se l'opzione scelta è uguale a 'eng'
    var language = 'en'; //la lingua di ricerca sarà in inglese
  }
  if (typeSelect == 'spa') {
    //se l'opzione scelta è uguale a 'spa'
    var language = 'es'; //la lingua di ricerca sarà in spagnolo
  }
  if (typeSelect == 'ger') {
    //se l'opzione scelta è uguale a 'ger'
    var language = 'de'; //la lingua di ricerca sarà in tedesco
  }
  if (typeSelect == 'fra') {
    //se l'opzione scelta è uguale a 'fra'
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.total_results > 0) {
          //se ci sono risultati nella ricerca
...
function creaTemplate(filmResults) {
  ...
  for (var i = 0; i < filmResults.length; i++) {
    //vado a scorrere per tutta la lunghezza dell'array
    filmResults[i];
    if (filmResults[i].hasOwnProperty('title')) {
      //se nell'array è definita la proprietà .title (e quindi è un Film)
      ...
      //se nell'array NON è definita la proprietà .title (e quindi se non è un film, è una SerieTV)
    ...
    if (filmResults[i].poster_path == null) {
      //se l'immagine di copertina non è presente
      ...
      var copertina =
        'https://image.tmdb.org/t/p/w342' + filmResults[i].poster_path; //creo una var che mi concatena l'url base dell'immagine e il contenuto dell'oggetto
    }
    if (filmResults[i].overview == '') {
      //se non c'è scrita nessuna descrizione del film/serietv
      var trama = ' Momentaneamente non disponibile/Temporarily unavailable'; //la var trama indica che non è disponibile
    } else {
      //altrimenti se c'è scritto qualcosa
...
function creaBandiere(flag) {
  if (flag == 'en') {
    //se ciò che andiamo a richiamare nella funzione corrisponde a 'en' allora mi mette al posto di en un'immagine (stessa cosa per tutti gli altri qui sotto)
    return 'img/uk.png';
  }
  if (flag == 'it') {
